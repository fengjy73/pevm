...
**Date:** 2026-09-14  
**Plant:** `82d9262` / `fbf328a` (PinHold snapped resume k≥8 + one-shot R1b + Ready|Executing refuse)  
...
| Weld | Status |
|------|--------|
| PinHold arms rem checkpoint → wake ResumeAtK | **code** (Unfenced snaps; first-access / k<8 FullRetry). Unit: `arm_pinhold_*` |
| R1b `try_arm_r1b_covered` without cp_k≥8 theater | **code** (strips only, **one** RewindTo). No arm → honest B0. Unit: `try_arm_r1b_covered_*` |
...
**Did not beat 0.703 / 0.449.** Soft=0 held. R1 is no longer theater.

### Focus N=3 @8 (`lab/results/v94-timely-resolve-focus.json` @ k≥8 + Ready-refuse)

| bn | sf_occ | parent | Wait | pin | R1 | refuse | park resume_k / full_retry | abort SF / OCC |
|---:|-------:|-------:|-----:|----:|---:|-------:|---------------------------:|---------------:|
...
Soft=0 on all four. Bind 0. Focus median **0.445**.
...
- R1 **1574 / 1581** (rate **0.996**) — Resolve conversion is real when strips cover.
- 24/98 ≥1.0; 37/98 ≥0.70; 35/98 <0.50.

## What cashed vs what did not

**Cashed:** R1 theater is dead (win≈attempt). 19807137 abort_SF **≤** OCC (1009/1066) and N=3 **0.269 > parent 0.231**. PinHold no longer livelocks (empty/k<8 FullRetry; R1b depth 1). Iter26 seq≡par held.

**Did not cash:** 14689597 still **0.276 ≪ 0.449** — PinHold FullRetry + extra abort (111 vs 41) + refuse 836. All-blocks median **0.600 ≪ 0.703**. Ready-refuse 31k on 19606599 is idle tax (dropping it lost the 19807137 beat). Tiny ResumeAtK was worse (0.17 / 0.09) so k≥8 FullRetry is the hang-free product path until cheap jump exists.

Do not celebrate pin↑ / refuse↑. Falsifier on 14689597 is still live: WaitFor↑ ∧ abort>OCC ∧ R1≈0.
