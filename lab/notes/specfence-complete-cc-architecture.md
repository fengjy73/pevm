...
chain admission / version-read pipeline, bounded optimism, fine-grained
...
Optimism is **bounded**. Known/predicted essential anti-deps → read-published or
ordered wait-for. **Never** Spec+retry as the contention response.
...
| A4 | D4 | Spine serialize; independence-certified Spec freely |
...
if published Data exists (incl. Executed-not-Validated tip):
    ...
    SpecRead                                   # A2 discovery
elif ℓ ∈ H / clique and writer known unpublished:
    WaitFor(writer)                            # A1 mass-Spec gate
else:
    SpecRead                                   # cold discovery only
```

Spec is **not** the response to a known essential anti-dep.
...
- Admit work along the spine: later readers of unpublished essential ℓ WaitFor
  the known writer; do not mass-Spec the clique.
- Independence-certified txs (no predicted essential edge) Spec freely.

---

## 5. First-wave intra (A2/D2)

- One canary Spec per hot ℓ before anyone has published.
...
- Spec only when **no** readable version exists.
