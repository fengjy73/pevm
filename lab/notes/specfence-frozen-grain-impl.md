...
| 5 | Timely PCC Avoid at **this** \(a\): Bind / WaitFor / serial-lane / ordered-admit iff PredictedEssential; mixed verbs in one tx | `edge.rs::classify_edge` (Data→Bind; PredictedEssential∧writer→WaitFor; else Unfenced); `vm.rs` WaitFor-only `admit_spine_writers_heat` | **landed** |
