...
| Before | After |
|--------|--------|
| Worker: `Occ ∨ plant_is_occ(empty PE ∨ quiet_pessimistic_off)` → `next_occ_task` else `next_sf_task` | **SpecFence always** `next_sf_task` (empty extras ≡ OCC walk). `Occ` mode remains a **separate** computer |
...
| Action | File |
|--------|------|
| **SPLIT** Fence policy out of `vm` | `specfence/fence_act.rs` (`FenceAct::{WaitForDependency,DoneOptimisticRead,ReadyCanary}`) |
| **SPLIT** wave product surface | `specfence/wave.rs` **owns** `WaveParkTable` + `ParkKind`/`WaitForDependency` (~530 LOC). SoftWait Soft + SuffixRepair remain in `rem.rs` (quarantined; Soft=0) |
...
| 3 | wait_for_dependency | `ParkKind::WaitForDependency`; WaitFor pending park is WaitForDependency; **no** steal-convert on WaitForDependency |
| 4 | decide←Bayes | `decide_queried` + `BayesMap::query_access`; known-star opens WaitFor on quiet morph |
| 5 | R1 live | `validate_specfence`: snap/FF `identity_stable_match` **or** incarnation-strict `prior_read_value_stable`; selective fenced subset; `record_partial_abort_win` |
| 6 | Cert survival | `begin_execute` no longer wipes locs on `inc==0`; `begin_block` is the only wipe |
| 7 | SerialLane | uses `act_serial_lane` (Done ≠ OrderedAdmit count) |
| 8 | Telemetry | `wait_for_dependency`, `wait_for_full_abort`, `ordered_admit_after_done`, `partial_abort_win`, `partial_abort_attempt`, `refuse_admit` |
| 9 | Quiet = optimistic_read cost class | empty PE → Mode(a)=Spec on **same** `next_sf` / `validate_specfence` symbols |

**OrderedAdmit-after-Done:** WaitFor/lane hitting a Done writer **certs** for R1 but **does not** `record_edge_ordered_admit` / `note_ordered_admit_success` (was the 442/473 theater).
...
| Suite | Result |
|-------|--------|
| `cargo test -p pevm --lib` | **202 passed** (incl. refuse inc==0, WaitForDependency, decide EV, admit ≥16) |
...
| partial_abort win ≥50% cert fan_out | **path live**; rate **not measured** |
| OrderedAdmit-after-Done <10% | **count path fixed** (Done≠OrderedAdmit); share **not measured** |
...
```
crates/pevm/src/pevm.rs          # unified next/execute/validate; admit_seed; WaitForDependency park
crates/pevm/src/vm.rs            # thin gate → decide_queried + fence_act; WaitForDependency park kind
crates/pevm/src/specfence/
  ...
  wave.rs                        # WaveParkTable + WaitForDependency (physical extract from rem)
...
One pevm spine, file-SRP, Bayes→admit→decide→WaitForDependency→R1 **duties landed** (refuse fires, WaitForDependency ≠ Aborting, decide consumes EV, PartialAbortRewind wired, admit seeds stars without prior gate). Soft=0, seq≡par tests green. Product TPS bars need a **new** Soft=0 sweep — this PR does not invent 0.95 or claim a crush of 0.685.
...
| Soft | 0 | **0** (held) |
| WaitFor / OrderedAdmit (N=1 agg) | — | 2151 / 2208 |
| partial_abort win/attempt | — | 6/1636 |
| OrderedAdmit-after-Done | — | 205 |
...
| Duty | Land |
|------|------|
| schedule-first Avoid | `try_execute_ready` refuses known consumers on **inc==0**; `record_refuse_admit_n` from `next_sf_task` |
| ProducerStage refuse | first wave respects ReadyEdge while `w` Executing; Ready/Validated still canary |
| wait_for_dependency | `add_wait_for_dependency` (status stays Executing); wake `set_wait_for_dependency_ready` same incarnation; pevm WaitForDependency arm does **not** `add_dependency` |
| BlockingOther not default | `fence_act::estimate_park_kind`; PE-known ESTIMATE / `fence_wait_for` → WaitForDependency |
| decide←Bayes | `ev_wait_for_dependency_beats_abort` / `depth_frac` are the WaitFor spine; OR-bool is no-query adapter only |
| R1 live | `query_validate` + `repair_grain`; PartialAbortRebind value-stable; PartialAbortRewind `apply_suffix_repair` when EV/covers |
...
| file-SRP | ESTIMATE park-kind + OrderedAdmit-rare EV in `fence_act`; `vm::fence_wait_for` museum; no `pc/cc/bayes` dirs |
| OrderedAdmit rare | `ordered_admit_ev_from_query`: `ev_ordered_admit_beats_full_abort` only — `known_star` is wait_for_dependency/WaitFor, not OrderedAdmit EV |
| true-k | admit plants Basic(addr) PE at k≈6 for hinted stars; `note_abort_access` skips any-k when class already seeded |

**Honesty (this package `c42f96a`):** Soft=0 all-blocks N=1 nonempty median **0.7033** (beats wall 0.6853); fan **14689597** N=3 **0.4485** (beats wall 0.3482). OrderedAdmit **0** / WaitFor wait_for_dependency **5158** vs aborting **177** / refuse **1540**. R1 **3/2439**. Product bars (median ≥0.95, fan ≥0.90, quiet p10 ≥0.90, R1 ≥50%) still **miss**. Artifacts: `lab/notes/v9.4-sot-partial-sweep-summary.json`, `lab/notes/specfence-v9.4-sweep-honesty.md`.
