...
| I | **Validate → R1 live** (fenced RAW + tip snap; R1 win ≥50% cert-bearing; R1b when EV) | **LANDED** (path; rate pending) | `validate_specfence`: `repair_grain` + `query_validate`; R1a value-stable; R1b `apply_suffix_repair` when EV/covers | Tip @ `3687da6` was 6/1636. Rate after this land **not** re-swept. |
...
|------:|------|--------|-------------------------------------|-----------|
| 1 | **Schedule-first refuse on incarnation 0** | **LANDED** (code; sweep pending) | `try_execute_ready` + steal path: no `incarnation > 0` gate; `ReadyEdgeTable::defer` → `record_schedule_refuse_n` in `next_sf_task` | unit: inc==0 refuse while `w` Executing; sweep `schedule_refuse ≫ 0` not yet re-run |
| 2 | **PinWithoutThrow = no Aborting** | **LANDED** (code; sweep pending) | `scheduler::add_pin_hold` / `set_pin_ready`; pevm PinHold arm **not** `add_dependency`; wake same incarnation; steal-convert still banned | unit: pin keeps Executing, resume inc==0; `waitfor_aborting ≪ waitfor_pin` needs sweep |
| 3 | **Kill / retarget BlockingOther WaitFor as default Avoid** | **LANDED** (code; sweep pending) | `fence_act::estimate_park_kind`; `vm::park_estimate_blocking` + `fence_wait_for` PE-known → PinHold; unknown ESTIMATE stays BlockingOther | waitfor_aborting vs pin needs sweep |
