...
def analyze_block(
    ...
    # This tree is e94b0e3. Native OCC does not emit per-tx probes, so the
    # critical-path schedule uses the workers=1 SpecFence profile instead.
    ideal_source = "occ"
    if not any(ln.get("attempts") for ln in seq_occ_lines):
        ideal_source = "sf"
        seq_occ_lines = [
            sample_block(seq_src, "sf")[r] for r in sorted(sample_block(seq_src, "sf"))
        ]
    ...
    out = {
        ...
        "same_timer": {
            "definition": (
                "OCC workers=1 successful ExecPhase.total_ns median per tx"
                if ideal_source == "occ"
                else "SpecFence workers=1 committed execution total_ns. Native OCC at e94b0e3 has no per-tx probe."
            ),
...
def curves_markdown(rows: list[dict]) -> str:
    ...
    lines = [
        ...
        "TPS_OCC is native pevm at e94b0e3 (Pevm::execute_revm_parallel). TPS_SF is the feature-gated SpecFence engine.",
        "",
    ]
...
def main() -> None:
    ...
    if args.curves:
        ...
        out = {
            ...
            "tps_seq_note": "TPS_SEQ is the workers=1 sequential median, not a per-C measurement. TPS_OCC is native pevm@e94b0e3 execute_revm_parallel. TPS_SF is the feature-gated SpecFence engine. TPS_ideal uses SpecFence workers=1 profile attempts when native OCC records none.",
        }
