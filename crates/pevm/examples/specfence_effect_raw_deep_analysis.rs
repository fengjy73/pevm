//! Deep effect-RAW instrumentation analysis (lab opt-in).
//!
//! Runs OCC@1 (serial-forced G* stream) + OCC@8 with FineGrain deep mode on
//! contiguous segments A/B/C. Writes:
//! - `lab/results/effect-raw-deep.{json,csv}`
//! - per-core `lab/results/effect-raw-deep-b{N}.json`
//!
//! Usage:
//! ```
//! cargo run -p pevm --release --config 'profile.release.lto=false' \
//!   --example specfence_effect_raw_deep_analysis
//! ```
...
fn effect_stats(snap: &FineGrainSnapshot) -> EffectRawStats {
    ...
    EffectRawStats {
        ...
        depth_p10: ma_md.depth_p10,
        depth_p50: ma_md.depth_p50,
        depth_p90: ma_md.depth_p90,
        depth_mean: ma_md.depth_mean,
        frac_depth_lt_0_01: ma_md.frac_depth_lt_0_01,
...
fn gap_note(bn: u64, stats: &EffectRawStats) -> String {
    ...
    if let Some(u) = user {
        format!(
            "block {bn}: effect_RAW={} (prog={}) vs user≈{u}; final_RW_RAW={}; unique_pcl={}; mean_effects/pcl={:.2}. Remaining gap likely: (1) mid-tx journal effects beyond DB::storage/basic calls, (2) write ordinal≈finalize write_set order not true SSTORE stream, (3) user may count WAW/internal or different location hashing, (4) aborted-incarnation edges included here.",
...
fn main() {
    let out = repo_root().join("lab/results/effect-raw-deep.json");
    ...
    for &bn in &all_blocks {
        ...
        eprintln!(
            "  occ@1 deep: {:.1}ms TPS={:.0} aborts={} edges={} ok={}",
            occ1.elapsed_ms,
            occ1.tps,
            occ1.occ_aborts,
        ...
        eprintln!(
            "  occ@8 deep: {:.1}ms TPS={:.0} aborts={} edges={} ok={}",
            occ8.elapsed_ms,
            occ8.tps,
            occ8.occ_aborts,
        ...
        eprintln!(
            "  effect_RAW={} (prog={} hand={}) final_RW={} depth_p50={:.4} frac≪1%={:.3} redo_saved={:.1} wait_added={:.1}",
            effect.n_raw_effect_total,
            effect.n_raw_effect_program,
            effect.n_raw_effect_handler,
            effect.n_raw_final_rw,
            effect.depth_p50,
            effect.frac_depth_lt_0_01,
            effect.redo_saved,
            effect.wait_added
        );
        ...
        if CORE_BLOCKS.contains(&bn) || NEIGHBOR_FOCUS.contains(&bn) {
            ...
            let deep_path = out.with_file_name(format!("effect-raw-deep-b{bn}.json"));
    ...
    user_gap.insert(
        "14689597".into(),
        "user≈3802 producer-effect→consumer-read; prior final-RW RAW=449".into(),
    );
    let payload = Out {
        generated: chrono_lite_now(),
        method_notes: vec![
            "OCC@1 preferred for clean G* effect stream (serial-forced parallel path).".into(),
            "Deep mode emits RawEffectEdge on every VmDb storage/basic/code_hash MV read.".into(),
            "producer_effect_k = index in producer finalize write_set (not live SSTORE stream).".into(),
            "depth_frac = first_program_cross_k / total_db_effects (true mid-tx gas needs inspect).".into(),
            "M-A = full redo per consumer with program cross; M-D redo∝(1-d); wait∝lag_norm*(1-steal).".into(),
            "Production default unchanged: finegrain_deep off.".into(),
        ],
    ...
    writeln!(
        csv,
        "seg,block,n_tx,occ1_tps,occ8_tps,occ8_abort,n_raw_effect,n_prog,n_hand,n_final_rw,unique_pcl,mean_eff_per_pcl,depth_p10,depth_p50,depth_p90,frac_lt_1pct,redo_saved,wait_added,eff_prog_path,final_rw_chain,fanout"
    )
    .unwrap();
    for s in &summaries {
        let e = &s.effect;
