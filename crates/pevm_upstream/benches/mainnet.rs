...
use pevm_upstream::{Pevm, chain::PevmEthereum};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    // Pin the criterion parallel group to the same C as the harness (taskset outside).
    let concurrency_level = std::env::var("PEVM_BENCH_CONCURRENCY")
        .ok()
        .and_then(|s| s.parse().ok())
        .and_then(NonZeroUsize::new)
        .unwrap_or(concurrency_level);
