...
2. **已完成** — (a)–(d) 对照。数字在 `docs/specfence-v2-stage2c.md`。mimalloc 两边都更慢；共享代码/缓存只减少基态读取次数。
3. **已完成** — 去掉解释器内 40×50µs 自旋；停车读者改跑未在执行的前驱；活跃集下限是 ready+executing；`pop` 用相位原子跳过过期项；目录代际和 abort 写者入链。
4. **已完成** — `specfence::` 37 例与三个集成测试通过。本机门未全过，已写入正式文档。草稿 PR #73。草稿待用户验收后删除。
